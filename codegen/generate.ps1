...
function Generate-Version {
    ...
    Write-Host "`nGenerating $Version provider..." -ForegroundColor Green
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to generate $Version provider"
        return $false
    }

    Write-Host "✓ Generated $Version provider" -ForegroundColor Green
    return $true
}
...
if ($success) {
    Write-Host "`n✓ All providers generated successfully!" -ForegroundColor Green
    Write-Host "Output directory: $outputDir" -ForegroundColor Cyan
} else {
    Write-Error "Some providers failed to generate"
    exit 1
}

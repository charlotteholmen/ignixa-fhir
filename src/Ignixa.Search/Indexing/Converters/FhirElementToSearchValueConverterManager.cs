...
/// </summary>
public class FhirElementToSearchValueConverterManager : IElementToSearchValueConverterManager
{
    private readonly Dictionary<(string fhirElementType, Type searchValueType), IElementToSearchValueConverter> _converterDictionary;

    public FhirElementToSearchValueConverterManager(IEnumerable<IElementToSearchValueConverter> converters)
    {
    ...
    internal class ExtensionConverter : IElementToSearchValueConverter
    {
        private readonly List<IElementToSearchValueConverter> _searchValueTypeConverters;

        public ExtensionConverter(Type searchValueSearchValueType, List<IElementToSearchValueConverter> searchValueTypeConverters)
        {
        ...
        public IEnumerable<ISearchValue> ConvertTo(IElement value)
        {
            ...
            IElement typed = value.Select("value").FirstOrDefault();
            IElementToSearchValueConverter converter = _searchValueTypeConverters.FirstOrDefault(x => x.FhirTypes.Contains(typed?.InstanceType));

...
- **Enable data linkage** across de-identified datasets (deterministic de-identification)
- **Support batch processing** of FHIR resources at scale
- **Maintain FHIR validity** after de-identification (structure preservation)
- **Be version-agnostic** (STU3, R4, R4B, R5, R6 support)

**Why existing solutions weren't sufficient**:

- Microsoft's FHIR-Tools-for-De-identification is tightly coupled to Firely SDK v3/v4 (incompatible with Ignixa's custom serialization)
- Hard-coded for specific FHIR versions (R4/STU3 only)
- Requires C# code changes to add custom de-identification rules
...
Implement **Ignixa.DeId** as a Core SDK library with the following architectural decisions:

```mermaid
flowchart TB
    subgraph "De-identifier Architecture"
        Config[De-identifierConfiguration<br/>JSON + FHIRPath rules]
        Engine[DeIdEngine<br/>Rule evaluation]
        Visitor[DeIdVisitor<br/>Tree traversal]
...
### 1. Hash-Based De-identification (vs Encryption)

**Decision**: Use HMAC-SHA256 as the default de-identification method for identifiers.
...
| AES-256-GCM (optional) | Reversible if key is retained | Not one-way; doesn't meet GDPR "de-identification" standard |
...
**Decision**: JSON configuration with FHIRPath expressions for de-identification rules.
...
**Why Visitor**:
- Stateful processing (track de-identification context via `_contextStack`)
- Clean separation: `DeIdVisitor` handles traversal, `ResourceProcessor` handles rules
- Extensibility: add new visitors without modifying core logic

### 4. Processor Factory Pattern

**Decision**: `IDeIdProcessorFactory` for extensibility with custom processors.

```csharp
public interface IDeIdProcessorFactory
{
    IDeIdProcessor CreateProcessor(string name, Dictionary<string, object>? settings);
}
```
...
**Trade-off**: No automatic key rotation. If key is compromised, all de-identified data must be re-processed with new key (breaks linkage to prior data).
...
**Decision**: Add `meta.security` labels to de-identified resources.
...
| Separate AuditEvent | Doesn't modify resource | Harder to track which resources were de-identified |
...
**Architecture**:
```
Input Stream → Partition 1 (Batch 100) → Deidentify → Output Stream
            → Partition 2 (Batch 100) → Deidentify → Output Stream
            → ...
            → Partition 8 (Batch 100) → Deidentify → Output Stream
```
...
- **No federated de-identification**: Single-engine only (no distributed processing)
...
- CLI tool (command-line batch processing) - **Already implemented** in `tools/Ignixa.DeId.Cli`

**CLI Tool Example**:
```bash
# Deidentify R4 resources
ignixa-de-identifier r4 anonymize \
  --input-folder ./raw-fhir \
  --output-folder ./de-identified \
...
- **Source**: Forked from [Microsoft FHIR-Tools-for-De-identification](https://github.com/microsoft/FHIR-Tools-for-De-identification)
...
- **Implementation**: PR #221 - Adds De-identifier library
- **CLI Tool**: `tools/Ignixa.DeId.Cli/Program.cs`
- **Configuration Examples**: `test/Ignixa.DeId.Tests/Configurations/common-config.json`

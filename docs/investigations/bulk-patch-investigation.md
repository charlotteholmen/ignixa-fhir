# Investigation: $bulk-patch Operation

**Date:** 2026-01-01
**Status:** In Progress
**Feature:** FHIR $bulk-patch / $bulk-update Operation

## Summary

Implement the `$bulk-patch` operation as defined by Microsoft Azure Health Data Services, enabling bulk updates to FHIR resources using FHIR Patch semantics.
...
Content-Location: https://server/_bulk-patch/{jobId}
```
...
1. **Endpoint** (`BulkPatchEndpoints.cs`) - Route registration
2. **Command/Handler** - `CreateBulkPatchJobCommand`, `GetBulkPatchJobStatusQuery`
3. **Orchestration** (`BulkPatchOrchestration.cs`) - DurableTask workflow
4. **Activities:**
   - `GetBulkPatchRangesActivity` - Partition resources by surrogate ID ranges
   - `BulkPatchWorkerActivity` - Apply patches in batches
...
src/
  Application/
    Ignixa.Api/
      Endpoints/
        BulkPatchEndpoints.cs           # Route registration
    Ignixa.Application/
      Features/
        BulkPatch/
          ...
          BulkPatchJobResult.cs
    Ignixa.Application.BackgroundOperations/
      BulkPatch/
        Orchestrations/
          BulkPatchOrchestration.cs
        Activities/
          GetBulkPatchRangesActivity.cs
          BulkPatchWorkerActivity.cs
        Models/
          BulkPatchJob.cs
          BulkPatchRange.cs
          BulkPatchWorkerResult.cs
```

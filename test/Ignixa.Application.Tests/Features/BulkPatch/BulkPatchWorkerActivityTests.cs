...
public class BulkPatchWorkerActivityTests
{
    ...
    private readonly ILogger<BulkPatchWorkerActivity> _logger;

    private readonly ISearchService _searchService;
    private readonly IFhirRepository _repository;
    private readonly IFhirSchemaProvider _schemaProvider;

    public BulkPatchWorkerActivityTests()
    {
        ...
        _logger = NullLogger<BulkPatchWorkerActivity>.Instance;
    ...
    {
        // Arrange
        var input = new BulkPatchWorkerInput
        {
            JobId = "job-123",
            TenantId = 1,
    ...
    private BulkPatchWorkerActivity CreateActivity()
    {
        return new BulkPatchWorkerActivity(
    ...
    private static BulkPatchWorkerInput CreateTestWorkerInput()
    {
        return new BulkPatchWorkerInput
        {
            JobId = "test-job-123",
            TenantId = 1,
            ResourceType = "Patient",
    ...
    private static async Task<BulkPatchWorkerOutput> InvokeActivityAsync(
        BulkPatchWorkerActivity activity,
        BulkPatchWorkerInput input)
    {
        var taskContext = new TaskContext(new OrchestrationInstance { InstanceId = "test" });
        var method = typeof(BulkPatchWorkerActivity)
            .GetMethod("ExecuteAsync", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);

        var task = (Task<BulkPatchWorkerOutput>)method.Invoke(activity, [taskContext, input]);
        return await task;
    }
}

public class BulkPatchWorkerConversionTests
{
    [Fact]

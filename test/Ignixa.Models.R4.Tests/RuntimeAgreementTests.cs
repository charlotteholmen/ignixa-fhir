...
/// <summary>
/// The typed facade view must AGREE with the schema-aware IElement / FHIRPath runtime over the SAME
/// node. Any divergence is a real bug because typed access and validation/FHIRPath would disagree.
/// Ported from the typed-models spike (RuntimeAgreementSpikeTests) against the graduated generated types.
/// </summary>
public sealed class RuntimeAgreementTests
{
    ...
    public void GivenPatient_WhenComparingBirthDate_ThenFacadeAgreesWithFhirPath()
    {
        ...
        var patient = resource.As<Ignixa.Models.R4.Patient>();
    ...
    public void GivenObservationQuantity_WhenComparingChoice_ThenFacadeAgreesWithFhirPath()
    {
        var resource = ResourceJsonNode.Parse(
            ...
            """);
        var obs = resource.As<Ignixa.Models.R4.Observation>();
        IElement element = resource.ToElement(Schema);

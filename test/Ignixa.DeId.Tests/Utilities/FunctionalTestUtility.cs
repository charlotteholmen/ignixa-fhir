...
public static class FunctionalTestUtility
{
        ...
        RequestOptions? settings = null)
    {
        ...
        var result = await engine.DeidentifyAsync(testContent, settings);

        result.IsSuccess.ShouldBeTrue($"DeId failed: {(result.IsSuccess ? "" : result.Error.Message)}");

        string standardizedResult = Standardize(result.Value.DeidentifiedJson);
